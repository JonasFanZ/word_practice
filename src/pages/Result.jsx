  ...
  return (
    <div className="w-[420px] bg-white rounded-2xl shadow-lg p-8 text-center space-y-6">
      ...
      <div className="flex flex-col gap-3">
        <button onClick={() => { dispatch({ type: "RESET_ALL" }); onHome(); }}
          className="w-full py-3 rounded-xl bg-gray-200 hover:bg-gray-300 transition">回初始畫面</button>
        <button onClick={handleContinue} className="w-full py-3 rounded-xl bg-blue-600 text-white hover:bg-blue-700 transition">繼續新的十題</button>
      </div>
    </div>
  );
}
