/**
 * 從 Google Sheet 讀題庫（公開可讀）
 * - 將 SHEET_ID 改為你的表單 ID
 * - 分頁命名：Level1 ~ Level6
 * 欄位：word | pos | meaning
 */
...
export async function fetchSheet(level) {
  if (SHEET_ID === "PUT_YOUR_GOOGLE_SHEET_ID_HERE") {
    // 開發階段用 mock
    return mockData;
  }
  try {
    return await fetchFromGoogle(level);
  } catch (e) {
    console.warn("Google Sheet 讀取失敗，改用 mock 資料", e);
    return mockData;
  }
}
